...
public class MemberController {
    ...
    @PostMapping("/GroupBuying/join")
        public String save(@ModelAttribute MemberDTO memberDTO, Model model) {  //회원가입에 필요한 정보를 DTO 객체로 받아왔다. (from join.html파일)
        System.out.println("MemberController.save");
        System.out.println("memberDTO = " + memberDTO);
        memberService.save(memberDTO); //memberService 객체의 save 메소드를 호출하면서 동시에 DTO 객체를 넘겼다.
        model.addAttribute("message", "가입이 완료되었습니다.");//추가
        model.addAttribute("searchUrl", "/");//추가
        return "message";
    }
